...
function initialize() {
  ...
  function getPlaybackRate() {
    ...
    chrome.storage.sync.get({ udemy_playback_rate: 1 }, obj => {
      // gotta love algebra
      const itemIndexPosition = (obj.udemy_playback_rate - MAX_RATE) / INCREMENT / -1;
  ...
  function populateItems() {
    ...
    const menuHtml = menuItems.reduce((acc, curr, index) => (acc += renderListItem(index)), "");
  ...
  if (rate === "increase" || rate === "decrease") {
    chrome.storage.sync.get({ udemy_playback_rate: 1 }, obj => {
      ...
      chrome.storage.sync.set({ udemy_playback_rate: newRate }, () => {
        ...
        document.querySelector("[data-purpose='playback-rate-button']").innerText = newRate;
      });
    });
  } else {
    chrome.storage.sync.set({ udemy_playback_rate: rate }, () => {
      video.playbackRate = rate;
      document.querySelector("[data-purpose='playback-rate-button']").innerText = rate;
    });
  }
}
  ...
  mutations.forEach(mutation => {
    ...
    if (mutation.addedNodes.length > 0) {
      ...
      addedNodes.forEach(node => {
        ...
        checkForClass(node, "playback-rate--playback-rate-value--3SJ7v", "rateBtn");
      });
    }
